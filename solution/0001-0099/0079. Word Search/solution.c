bool hasNestedNeighbor(int i, int j, char* word, char** board, bool** passedBoard, int boardSize, int* boardColSize) {
  if (passedBoard[i][j] || board[i][j] != *word) { return false; }
  if (*(word + 1) == '\0') { return true; }
  passedBoard[i][j] = true;
  if (i > 0 && hasNestedNeighbor(i - 1, j, word + 1, board, passedBoard, boardSize, boardColSize)) { return true; }
  if (i < (boardSize - 1) && hasNestedNeighbor(i + 1, j, word + 1, board, passedBoard, boardSize, boardColSize)) {
    return true;
  }
  if (j > 0 && hasNestedNeighbor(i, j - 1, word + 1, board, passedBoard, boardSize, boardColSize)) { return true; }
...
bool exist(char** board, int boardSize, int* boardColSize, char* word) {
  ...
  for (int i = 0; i < boardSize; i++) { passedBoard[i] = calloc(boardColSize[i], sizeof(bool)); }
  for (int i = 0; i < boardSize; i++) {
